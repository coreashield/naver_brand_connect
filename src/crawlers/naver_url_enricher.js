...
const DELAY_BETWEEN_PRODUCTS = 3000;  // 상품 간 딜레이 (ms)
...
async function main() {
  ...
  try {
    ...
    while (true) {
      ...
      // 중간 통계
      log(`\n📊 진행 상황: ${totalProcessed}개 처리 (성공: ${totalSuccess}, 실패: ${totalFailed})`);
    }
    ...
    console.log(`║   URL 보유율: ${Math.round(finalStats.withNaverUrl / finalStats.total * 100)}%`);
    console.log('╚════════════════════════════════════════╝\n');
