/**
 * Full Sync - Supabase 버전
 * 링크 발급 + 크롤링 + Supabase DB 동기화
 */
...
// 결과 저장 (JSON + CSV + Supabase)
async function saveResults(productsMap) {
  ...
  try {
    ...
    for (let i = 0; i < products.length; i += batchSize) {
      ...
      if (synced % 500 === 0) {
        console.log(`    ${synced}/${products.length} 동기화 완료`);
      }
    }

    console.log(`  ✅ Supabase 동기화 완료 (${synced}개)`);
  } catch (e) {
    console.error(`  ⚠️ Supabase 동기화 오류: ${e.message}`);
  }
...
async function main() {
  console.log('╔════════════════════════════════════════════════╗');
  console.log('║   Full Sync - Supabase 버전                    ║');
  console.log('║   1. 모든 카테고리에서 신규 상품 링크 발급     ║');
  console.log('║   2. 발급 링크 관리에서 데이터 수집            ║');
  console.log('║   3. JSON + Supabase DB 동기화                 ║');
  ...
  if (!connTest.success) {
    console.log(`⚠️ Supabase 연결 실패: ${connTest.error}`);
    console.log('   JSON 저장만 진행됩니다.');
  } else {
    console.log(`✅ Supabase 연결 성공 (기존 상품: ${connTest.productCount}개)\n`);
  }
  ...
  try {
    ...
    // 최종 저장
    const total = await saveResults(productsMap);

    // Supabase 상품 수 확인
    let supabaseCount = 0;
    if (connTest.success) {
      try {
        supabaseCount = await getProductCount();
      } catch (e) {}
    }

    syncLog.crawled = totalNew;
    syncLog.total = total;
    syncLog.supabaseTotal = supabaseCount;
    ...
    console.log(`║   로컬 상품: ${syncLog.total}개`);
    if (connTest.success) {
      console.log(`║   Supabase 상품: ${supabaseCount}개`);
    }
    console.log('╚════════════════════════════════════════════════╝\n');
